...
(function () {
  ...
  function parseMotisResponse(d) {
    ...
    ptItins.forEach(function(it) {
      ...
      var itTransfers = Math.max(0, it.legs.filter(function(l){var m=(l.mode||'').toUpperCase();return m!=='WALK'&&m!=='BICYCLE'&&m!=='CAR';}).length - 1);
      var exTransfers = existing ? Math.max(0, existing.legs.filter(function(l){var m=(l.mode||'').toUpperCase();return m!=='WALK'&&m!=='BICYCLE'&&m!=='CAR';}).length - 1) : 99;
    ...
    var trains = deduped.slice(0, 3).map(function(it) {
      ...
      // mainLegs: intercity/regional rail only — used for card header (dep, arr, train name).
      // Excludes metro/tram/subway/bus which are last-mile legs added by MOTIS.
      var mainLegs = it.legs.filter(function(l) {
        return MAINLINE.indexOf((l.mode || '').toUpperCase()) >= 0;
      });

      // allPtLegs: all non-walk PT — used for transfer count and leg breakdown.
      // Includes metro/tram/subway/bus so connections to urban destinations show correctly.
      var allPtLegs = it.legs.filter(function(l) {
        var m = (l.mode || '').toUpperCase();
        return m !== 'WALK' && m !== 'BICYCLE' && m !== 'CAR';
      });

      // Fallback: pure urban itinerary (no mainline at all) — use allPtLegs for header too
      var ptLegs = mainLegs.length ? mainLegs : allPtLegs;
      var first = ptLegs[0] || {};
      var last  = allPtLegs[allPtLegs.length - 1] || ptLegs[ptLegs.length - 1] || {};
      ...
      var legDetails = numTransfers > 0 ? allPtLegs.map(function(l, idx) {
        ...
        var waitMin = 0;
        if (idx > 0) {
          var prevArr  = (allPtLegs[idx-1].to && allPtLegs[idx-1].to.arrival) || allPtLegs[idx-1].endTime || '';
      ...
      return {
        ...
        realTime:  allPtLegs.some(function(l) { return l.realTime; }),
        legs:      legDetails
      };
    });
  ...
  function analyze() {
    ...
    function stepRender(ctx) {
      ...
      var canonKey = ANALYSIS_CACHE.key(ctx.oGeo.name, ctx.dGeo.name, dayOffset(), cacheHour);
